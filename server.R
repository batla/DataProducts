...
# following data is from # this is repdata-data-StormData.csv.bz2 data - 
# cleaned up and aggregated by year/state, events are counted and damage
# is summed
stormDataAgg <- fread('data/stormData_agg.csv') 
stormDataAgg$EVTYPE <- tolower(stormDataAgg$EVTYPE)
evtypes <<- sort(unique(stormDataAgg$EVTYPE))

shinyServer(function(input, output) 
{
  stormDataAgg.agg <- reactive({
    tmp <- merge(
      data.table(STATE=sort(unique(stormDataAgg$STATE))),stormDataAgg[YEAR >= input$range[1] & YEAR <= input$range[2] & EVTYPE %in% input$evtypes,
  ...
  stormDataAgg.agg.year <- reactive({stormDataAgg[YEAR >= input$range[1] & YEAR <= input$range[2] & EVTYPE %in% input$evtypes,
...
output$economicImpactByState <- renderPlot({
  ...
  title <- paste("Economic impact", input$range[1], "-", input$range[2], "(Millions USD)")
  p <- ggplot(data, aes(map_id = STATE))
...
dataTable <- reactive(
{
  stormDataAgg.agg()[, list(State=state.abb[match(STATE, tolower(state.name))],
...
output$eventsByYear <- renderChart({
  data <- stormDataAgg.agg.year()[, list(COUNT=sum(COUNT)), by=list(YEAR)]
  setnames(data, c('YEAR', 'COUNT'), c("Year", "Count"))
  
  eventsByYear <- nPlot(Count ~ Year,data = data[order(data$Year)],
...
output$populationImpact <- renderChart({
  data <- melt(stormDataAgg.agg.year()[, list(Year=YEAR, Injuries=INJURIES, Fatalities=FATALITIES)],id='Year')
...
output$economicImpact <- renderChart({
  data <- melt(stormDataAgg.agg.year()[, list(Year=YEAR, Propety=PROPDMG, Crops=CROPDMG)],id='Year')
